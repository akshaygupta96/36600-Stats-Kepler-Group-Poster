...
```{r}
set.seed(101)
df <- read.csv('/Users/mariusnwobi/Downloads/OSTATS/poster/kepler.csv', stringsAsFactors = TRUE)
dim(df)
summary(df)
```
...
# pred <- ifelse(prob > 0.5, 1, 0)
roc_svm <- roc(df.test$label, prob)
results <- get_scores(df.test$label, prob)
...
roc_knn <- roc(df.test$label, prob)
results <- get_scores(df.test$label, prob)
...
# pred <- ifelse(prob > 0.5, 1, 0)
roc_gbm <- roc(df.test$label, prob)
...
plot(roc_lr, col = "blue", lwd = 2, 
     xlim = c(1, 0), ylim = c(0, 1),  # Reverse x-axis
...
lines(roc_gbm,col = "black", lwd = 2)
lines(roc_xg,col = "gold", lwd = 2)



# Add a legend
#legend("bottomright", legend = c('Logistic Regression',"Random Forest", #"SVM","KNN",'GBM','XG'),
 #      col = c("blue", "red",'green','purple','black','gold'), lwd = 2)

text(0, 0.7, paste("AUC (Logistic Regression):", round(auc(roc_lr), 3)), col = "blue")
text(0, 0.6, paste("AUC (Random Forest):", round(auc(roc_rf), 3)), col = "red")
text(0, 0.5, paste("AUC (SVM):", round(auc(roc_svm), 3)), col = "green")
text(0, 0.4, paste("AUC (KNN):", round(auc(roc_knn), 3)), col = "purple")
text(0, 0.3, paste("AUC (GBM):", round(auc(roc_gbm), 3)), col = "black")
text(0, 0.2, paste("AUC (XG):", round(auc(roc_xg), 3)), col = "gold")
```
